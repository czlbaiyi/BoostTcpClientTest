...
//启动Tcp客户端
void startZLTcpClient(std::string ip, std::string port)
{
...
{
	boost::thread t1(boost::bind(&startZLTcpClient, ip,port));//创建一个分支线程，回调到myThread函数里	
}

void startSyncClient(std::string ip, std::string port)
{
	startZLTcpClient(ip,port);
}
